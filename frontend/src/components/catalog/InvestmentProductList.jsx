  ...
  return (
    <div className="card product-list-card">
      ...
      <div className="product-list">
        {products.map((product) => <button className="product-row" key={product.id} onClick={() => select(product)}><span className="product-icon" /><span><strong>{product.title}</strong><small>{product.description || `Riesgo ${product.risk} · ${product.rate}% anual`}</small></span><i>›</i></button>)}
