export default function RiskProfileSelector({ currentProfile, options = [], actions = [], onAction }) {
  ...
  return (
    <div className="card profile-selector-card">
      ...
      <div className="profile-options">{options.map((option) => <button key={option.id} className={`profile-option ${currentProfile === option.id ? "selected" : ""}`} onClick={() => select(option)}><strong>{option.title}</strong><small>{option.description}</small></button>)}</div>
    </div>
  );
}
