  ...
  return (
    <div className="phone-shell investment-shell">
      ...
      <header className="app-header shell-header">
        <div><Brand compact /><div className="tagline">{PAGE_COPY[page] || PAGE_COPY.home}</div></div>
        <div className="header-actions">
          <span className="live-agent-dot" title="Agente de inversiones disponible" />
          <button className="profile-mini" onClick={() => onNavigate("profile")} aria-label="Abrir perfil">{initial}</button>
        </div>
      </header>
      {children}
      <ChatInput onSend={onSend} disabled={loading} navigation={<BottomNav activeTab={page} onChange={onNavigate} />} />
    </div>
  );
}
