"""Ceremonias WebAuthn para biometria de plataforma."""
import json
...
def complete_registration(user: User, credential: dict, platform: str, device_name: str | None, db: Session) -> AuthDevice:
...
def authentication_options(user: User, db: Session) -> dict:
    ...
    options = generate_authentication_options(
        rp_id=settings.webauthn_rp_id,
        allow_credentials=[PublicKeyCredentialDescriptor(id=base64url_to_bytes(device.credential_id)) for device in devices],
        user_verification=UserVerificationRequirement.REQUIRED,
    )
